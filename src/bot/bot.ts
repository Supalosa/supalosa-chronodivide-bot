...
import {
  ...
  getPointTowardsOtherPoint,
} from "./logic/map/map.js";
...
export class ExampleBot extends Bot {
  ...
  override onGameStart(game: GameApi) {
    ...
    this.enemyPlayers = game
      .getPlayers()
      .filter((p) => p !== this.name && !game.areAlliedPlayers(this.name, p));
    ...
    this.logBotStatus(
      `Map bounds: ${this.knownMapBounds.x}, ${this.knownMapBounds.y}`,
    );
  }
  ...
  private numBuildingsOwnedOfType(
    game: GameApi,
    rule: (r: TechnoRules) => boolean,
  ): number {
  ...
  private updateBuildQueues(game: GameApi, playerData: PlayerData) {
    ...
    const decisions = queues
      .map((queueType) => {
        ...
        return {
          queue: queueType,
          decision: this.getBestOptionForBuilding(
            game,
            options,
            this.threatCache,
            false,
    ...
    if (queueData.status == QueueStatus.Idle) {
      ...
      if (queueType == QueueType.Structures || queueType == QueueType.Armory) {
        let location: { rx: number; ry: number } | undefined =
          this.getBestLocationForStructure(game, objectReady);
      ...
      queueData.status == QueueStatus.Active &&
      queueData.items.length > 0 &&
      decision != null
    ) {
      ...
      if (myCredits >= totalCostAcrossQueues) {
        this.logBotStatus(
          `Resuming queue ${queueData.type} because credits are high`,
        );
  ...
  private getBestOptionForBuilding(
    ...
    options.forEach((option) => {
      let priority = this.getPriorityForBuildingOption(
        option,
        game,
        playerStatus,
        threatCache,
    ...
    if (priorityQueue.length > 0) {
      ...
      if (debug) {
        let queueString = priorityQueue
          .map((item) => item.unit.name + "(" + item.priority + ")")
          .join(", ");
  ...
  private getPriorityForBuildingOption(
    ...
    if (buildingNameToAiBuildingRules.has(option.name)) {
      ...
      return (
        defaultBuildingPriority -
        this.numBuildingsOwnedOfType(game, (r) => r == option)
      );
    }
  }
  ...
  private getBestLocationForStructure(
    ...
    if (buildingNameToAiBuildingRules.has(objectReady.name)) {
      ...
      return getDefaultPlacementLocation(
        game,
        playerStatus,
        playerStatus.startLocation,
        objectReady,
      );
    }
  }
  ...
  override onGameTick(game: GameApi) {
    if (game.getCurrentTick() % this.tickRatio === 0) {
      ...
      this.sectorCache?.updateSectors(
        game.getCurrentTick(),
        sectorsToUpdatePerCycle,
        game.mapApi,
        myPlayer,
      );
      let updateRatio = this.sectorCache?.getSectorUpdateRatio(
        game.getCurrentTick() - game.getTickRate() * 60,
      );
      if (updateRatio && updateRatio < 1.0) {
        this.logBotStatus(
          `${updateRatio * 100.0}% of sectors updated in last 60 seconds.`,
        );
      }
      ...
      let shouldAttack = this.threatCache
        ? this.isWorthAttacking(this.threatCache, boredomFactor)
        : false;
      if (game.getCurrentTick() % (this.tickRatio * 150) == 0) {
        let visibility = this.sectorCache?.getOverallVisibility();
        if (visibility) {
          this.logBotStatus(
            `${visibility * 100.0}% of tiles visible. Calculating threat.`,
          );
      ...
      const armyUnits = game.getVisibleUnits(
        this.name,
        "self",
        (r) => r.isSelectableCombatant,
      );
      ...
      switch (this.botState) {
        case BotState.Initial: {
          ...
          let conYards = game.getVisibleUnits(
            this.name,
            "self",
            (r) => r.constructionYard,
          );
          if (conYards.length) {
            this.botState = BotState.Deployed;
            break;
          }
          const units = game.getVisibleUnits(this.name, "self", (r) =>
            baseUnits.includes(r.name),
          );
          if (units.length) {
        ...
        case BotState.Scouting: {
          ...
          const unseenStartingLocations = game.mapApi
            .getStartingLocations()
            .filter((startingLocation) => {
              if (
                startingLocation == game.getPlayerData(this.name).startLocation
              ) {
                return false;
              }
              let tile = game.mapApi.getTile(
                startingLocation.x,
                startingLocation.y,
              );
          ...
          armyUnits.forEach((unitId) => {
            if (candidatePoints.length > 0) {
              ...
              if (unit?.isIdle) {
                const scoutLocation =
                  candidatePoints[
                    Math.floor(Math.random() * candidatePoints.length)
                  ];
                this.actionsApi.orderUnits(
                  [unitId],
  ...
  private isWorthAttacking(threatCache: GlobalThreat, threatFactor: number) {
    ...
    let scaledGroundPower = Math.pow(
      threatCache.totalAvailableAntiGroundFirepower,
      1.125,
    );
    let scaledGroundThreat =
      (threatFactor * threatCache.totalOffensiveLandThreat +
        threatCache.totalDefensiveThreat) *
      1.1;

    let scaledAirPower = Math.pow(threatCache.totalAvailableAirPower, 1.125);
    let scaledAirThreat =
      (threatFactor * threatCache.totalOffensiveAntiAirThreat +
        threatCache.totalDefensiveThreat) *
      1.1;

    return (
      scaledGroundPower > scaledGroundThreat || scaledAirPower > scaledAirThreat
    );
  }
  ...
  private isHostileUnit(game: GameApi, unitId: number) {
    ...
    return (
      unitData.owner != this.name &&
      game.getPlayerData(unitData.owner)?.isCombatant
    );
  }
      ...
      case ApiEventType.ObjectDestroy: {
        ...
        if (ev.attackerInfo?.playerName == this.name) {
          this.tickOfLastAttackOrder +=
            (this.gameApi.getCurrentTick() - this.tickOfLastAttackOrder) / 2;
        }
        break;
      }
      default:
