...
export class BasicAirUnit implements AiBuildingRules {
  ...
  getPriority(
    ...
    if (threatCache) {
      ...
      if (
        this.antiAirPower > 0 &&
        threatCache.totalOffensiveAirThreat >
          threatCache.totalAvailableAntiAirFirepower
      ) {
        priority +=
          this.basePriority *
          (threatCache.totalOffensiveAirThreat /
            Math.max(1, threatCache.totalAvailableAntiAirFirepower));
      }
      ...
      priority += Math.min(
        ...
        Math.max(
          1,
          Math.sqrt(
            threatCache.totalAvailableAirPower /
              Math.max(1, threatCache.totalOffensiveAntiAirThreat),
          ),
        ),
      );
