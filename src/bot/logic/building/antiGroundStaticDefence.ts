...
export class AntiGroundStaticDefence implements AiBuildingRules {
  ...
  getPlacementLocation(
    ...
    let selectedLocation =
      enemyFacingLocationCandidates[
        Math.floor(game.generateRandom() * enemyFacingLocationCandidates.length)
      ];
  ...
  getPriority(
    ...
    if (threatCache) {
      ...
      if (threatCache.totalOffensiveLandThreat > denominator) {
        return (
          this.basePriority *
          (threatCache.totalOffensiveLandThreat / Math.max(1, denominator))
        );
      }
    }
