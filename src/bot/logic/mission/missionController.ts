...
export class MissionController {
  ...
  public onAiUpdate(
    ...
    const isDisband = (a: MissionAction): a is MissionActionDisband =>
      a.type == "disband";
    ...
    this.missions.filter(
      (missions) => !disbandedMissions.has(missions.getUniqueName()),
    );
    ...
    this.missions.forEach((mission) =>
      missionNames.add(mission.getUniqueName()),
    );
    missionFactories.forEach((missionFactory) => {
      let maybeMission = missionFactory.maybeCreateMission(
        gameApi,
        playerData,
        threatData,
        this.missions,
