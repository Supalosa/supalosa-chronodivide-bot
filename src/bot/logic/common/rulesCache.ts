...
// A WeakMap keyed by GameApi instance holds per-match caches.
const technoRulesCacheMap: WeakMap<GameApi, { [rulesName: string]: TechnoRules | null }> = new WeakMap();
...
export const getCachedTechnoRules = (gameApi: GameApi, unitId: number): TechnoRules | null => {
    ...
    // Obtain or initialize the per-game cache for TechnoRules
    let technoRulesCache = technoRulesCacheMap.get(gameApi);
    if (!technoRulesCache) {
        technoRulesCache = {};
        technoRulesCacheMap.set(gameApi, technoRulesCache);
    }
