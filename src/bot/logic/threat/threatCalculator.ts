import { GameApi, ObjectType, PlayerData, UnitData } from "@chronodivide/game-api";
...
export function calculateGlobalThreat(game: GameApi, playerData: PlayerData, visibleAreaPercent: number): GlobalThreat {
    ...
    let airUnits = game.getVisibleUnits(playerData.name, "hostile", r => (r.type == ObjectType.Aircraft));
    let groundDefence = game.getVisibleUnits(playerData.name, "hostile", r => (r.type == ObjectType.Building)).filter(unitId => isAntiGround(game, unitId));
    
    let ourAntiGroundUnits = game.getVisibleUnits(playerData.name, "self", r => (r.isSelectableCombatant)).filter(unitId => isAntiGround(game, unitId));
    let ourAntiAirUnits = game.getVisibleUnits(playerData.name, "self", r => (r.isSelectableCombatant)).filter(unitId => isAntiAir(game, unitId));
    let ourGroundDefence = game.getVisibleUnits(playerData.name, "self", r => (r.type == ObjectType.Building)).filter(unitId => isAntiGround(game, unitId));
...
function calculateFirepowerForUnit(unitData: UnitData): number {
    ...
    if (unitData.primaryWeapon) {
        threat += ((unitData.primaryWeapon.rules.damage + 1) * Math.sqrt(unitData.primaryWeapon.rules.range + 1)) / Math.max(unitData.primaryWeapon.cooldownTicks, 1);
    }
    if (unitData.secondaryWeapon) {
        threat += ((unitData.secondaryWeapon.rules.damage + 1) * Math.sqrt(unitData.secondaryWeapon.rules.range + 1)) / Math.max(unitData.secondaryWeapon.cooldownTicks, 1);
    }
