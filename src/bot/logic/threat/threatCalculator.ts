...
export function calculateGlobalThreat(game: GameApi, playerData: PlayerData, visibleAreaPercent: number): GlobalThreat {
    ...
    let antiAirPower = game.getVisibleUnits(playerData.name, "hostile", r => (r.type != ObjectType.Building)).filter(unitId => isAntiAir(game, unitId));
...
function calculateFirepowerForUnit(unitData: UnitData): number {
    ...
    if (unitData.primaryWeapon) {
        threat += hpRatio * ((unitData.primaryWeapon.rules.damage + 1) * Math.sqrt(unitData.primaryWeapon.rules.range + 1)) / Math.max(unitData.primaryWeapon.cooldownTicks, 1);
    }
    if (unitData.secondaryWeapon) {
        threat += hpRatio * ((unitData.secondaryWeapon.rules.damage + 1) * Math.sqrt(unitData.secondaryWeapon.rules.range + 1)) / Math.max(unitData.secondaryWeapon.cooldownTicks, 1);
    }
