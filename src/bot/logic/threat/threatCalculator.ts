...
export function calculateGlobalThreat(
  game: GameApi,
  ...
  let airUnits = game.getVisibleUnits(
    playerData.name,
    "hostile",
  ...
  let ourGroundDefencePower = calculateFirepowerForUnits(
    game,
    ourGroundDefence,
  );
    ...
    threat +=
      (hpRatio *
        ((unitData.primaryWeapon.rules.damage + 1) *
          Math.sqrt(unitData.primaryWeapon.rules.range + 1))) /
    ...
    threat +=
      (hpRatio *
        ((unitData.secondaryWeapon.rules.damage + 1) *
          Math.sqrt(unitData.secondaryWeapon.rules.range + 1))) /
