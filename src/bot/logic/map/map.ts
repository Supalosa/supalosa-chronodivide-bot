...
export function getPointTowardsOtherPoint(
  ...
  let radius =
    minRadius + Math.round(gameApi.generateRandom() * (maxRadius - minRadius));
  let directionToSpawn = Math.atan2(
    endLocation.y - startLocation.y,
    endLocation.x - startLocation.x,
  );
  let randomisedDirection =
    directionToSpawn -
    (randomAngle * (Math.PI / 12) +
      2 * randomAngle * gameApi.generateRandom() * (Math.PI / 12));
  let candidatePointX = Math.round(
    startLocation.x + Math.cos(randomisedDirection) * radius,
  );
  let candidatePointY = Math.round(
    startLocation.y + Math.sin(randomisedDirection) * radius,
  );
