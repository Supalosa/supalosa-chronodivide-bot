...
export class SquadController {
  ...
  public onAiUpdate(
    ...
    this.squads = this.squads.filter(
      (squad) => squad.getLiveness() == SquadLiveness.SquadDead,
    );
    ...
    this.squads.forEach((squad) => {
      squad.getUnitIds().forEach((unitId) => {
        if (this.unitIdToSquad.has(unitId)) {
          console.log(
            `WARNING: unit ${unitId} is in multiple squads, please debug.`,
          );
        } else {
    ...
    const isDisband = (a: SquadAction): a is SquadActionDisband =>
      a.type == "disband";
    const isMerge = (a: SquadAction): a is SquadActionMergeInto =>
      a.type == "mergeInto";
    ...
    squadActions
      ...
      .forEach((a) => {
        ...
        a.squad
          .getUnitIds()
          .forEach((unitId) => mergeInto.mergeInto.addUnit(unitId));
        disbandedSquads.add(a.squad.getName());
      });
