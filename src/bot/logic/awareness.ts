...
;
    ...
    onAiUpdate(game: GameApi, playerData: PlayerData): void {
        ...
        const boredomFactor =
            1.0 -
            Math.min(1.0, Math.max(0.0, (game.getCurrentTick() - this.tickOfLastAttackOrder) / 1600.0));
        ...
        if (game.getCurrentTick() % (600) == 0) {
            ...
            if (visibility) {
                ...
                this.logger(
                    ...
                    )}, us: ${Math.round(this.threatCache.totalAvailableAntiGroundFirepower)}.`
                );
                this.logger(
        ...
        if (game.getCurrentTick() % RALLY_POINT_UPDATE_INTERVAL_TICKS === 0) {
            const enemyPlayers = game.getPlayers().filter((p) => p !== playerData.name && !game.areAlliedPlayers(playerData.name, p));
